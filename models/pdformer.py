...
class PDFormer(AbstractTrafficStateModel):
    def __init__(self, config, pattern_keys=None, adj_mx=None, sd_mx=None, sh_mx=None, dtw_matrix=None, geo_num_heads=4,sem_num_heads=2, t_num_heads=2,
                                mlp_ratio=4, qkv_bias=True, drop=0.0, attn_drop=0.0, drop_path=0.3, enc_depth=6, type_ln='pre', 
                                output_dim=1, add_time_in_day=True, add_day_in_week=True, world_size=1, huber_delta=1, quan_delta=0.25, far_mask_delta=5,
                                dtw_delta=5, step_size=1562, max_epoch=200):
        super().__init__(config)
        pattern_keys = pattern_keys
        self.num_nodes = config.num_nodes
        self.device = config.gpu
        self.feature_dim = config.feat_dims[0]
        ...
        self.pattern_keys = torch.from_numpy(pattern_keys).float().to(self.device)
        ...
        self.enc_embed_layer = DataEmbedding(
            self.feature_dim, self.embed_dim, self.lape_dim, self.adj_mx, drop=drop,
