...
class MTGnn(nn.Module):
    ...
    def __init__(self, config, gcn_true=True, buildA_true=True, gcn_depth=2, predefined_A=None, 
        ...
        self.start_conv = nn.Conv2d(
            in_channels=self.in_dim, out_channels=residual_channels, kernel_size=(1, 1))
        self.gc = graph_constructor(
            self.num_nodes, subgraph_size, node_dim, alpha=tanhalpha, static_feat=static_feat)
            ...
            for j in range(1, layers+1):
                ...
                if self.seq_length > self.receptive_field:
                    self.norm.append(LayerNorm(
                        (residual_channels, self.num_nodes, self.seq_length - rf_size_j + 1), elementwise_affine=layer_norm_affline))
                else:
                    self.norm.append(LayerNorm(
                        (residual_channels, self.num_nodes, self.receptive_field - rf_size_j + 1), elementwise_affine=layer_norm_affline))
        ...
        if self.seq_length > self.receptive_field:
            self.skip0 = nn.Conv2d(in_channels=self.in_dim, out_channels=skip_channels, kernel_size=(
    ...
    def forward(self, history_data, idx=None):
        ...
        """
        # select feature
        history_data = history_data[..., :self.in_dim].transpose(1, 3).contiguous()
