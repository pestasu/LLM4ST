...
from utils.serialization import load_graph_data
from models import *
...
class Exp_gpt_st(Exp_Basic):
    ...
    def _build_model(self): 
        ...
        if self.args.mode == 'pretrain':
            ...
            A = load_graph_data(f'{self.args.data_root_path}/adj_mx.pkl')
    ...
    def train_batch(self, x, y, epoch):
        ...
        if self.mode == 'pretrain':
            label = x
            out, out_time, mask, probability, eb = self.model(x, label, self.batch_seen, epoch)
            pred, true = self._inverse_transform([out, label[..., :1]])
            loss_flow = self.loss_fn(pred, true, mask)
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y) in enumerate(self.dataloader['valid']):
                ...
                if self.args.mode == 'pretrain':
                    ...
                    pred, true = self._inverse_transform([output, batch_x[...,:1]])
